...
library(waffle) # install.packages("waffle")
library(fontawesome) # install.packages("fontawesome") - for waffle graphs glyphs
...
The purpose of this visualization is to explore the spatial economics of China's Special Economic Zones or SEZs. These areas benefit from relaxed regulation and government assistance in the form of subsides, tax breaks, or even direct intervention.[^6]
...
ggplot() + 
  ...
  theme_void() + 
  labs( 
    title = "You scream, I scream—We all scream for Chinese SEZs!",
    subtitle = str_wrap("The use of Special Economic Zones (SEZs) has led to approximately 20 percent of China's Gross Domestic Product to be centered in the southwest."),
...
```{r}
#| label: gdp by province 1992-2023

# have to render on macbook since I don't have this file on main machine
...
ggplot() + 
  geom_line(data = filter(province_gdp92_23, sez_province == 1),aes(x = year, y = annual_gdp, color = Region)) + 
  labs(
...
## BONUS: Visualization 7: Chinese GDP over time 

### Purpose 

The purpose of this visualization is to show how China's GDP has moved over time with respect to the establishment of the first special economic zones in 1980.

```{r}
#| label: visualization 2 code

china_50yeargdp <- 
  wb_data("NY.GDP.MKTP.CD", country = "China") |> 
  clean_names() |> 
  select(-unit, -obs_status, -footnote, -last_updated) |> 
  mutate(gdp_us_billions = ny_gdp_mktp_cd/10e9)

china_50yeargdp

ggplot(data = china_50yeargdp, aes(x = date, y = gdp_us_billions)) + 
  geom_line() + 
